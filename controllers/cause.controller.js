...
const fs = require('fs');
const causeModel = require('../models/cause.model');
...
module.exports = {
    ...
    createCause: async (req, res) => {
        try {
            ...
            form.parse(req, async (err, fields, files) => {
                ...
                const newCause = await causeModel.createCause(formData);
    ...
    getCauseById: async (req, res) => {
        try {
            ...
            const action = await causeModel.getCauseById(actionId);
    ...
    getCauses: async (req, res) => {
        try {
            ...
            all = await causeModel.getCauses(page, per_page, whereClause);
    ...
    updateCauseById: async (req, res) => {
        try {
            const form = new formidable.IncomingForm();
            const { id } = req.params;
            const fieldsToUpdate = {};
            const response = await causeModel.getCauseById(id);
            ...
            form.parse(req, async (err, fields, files) => {
                ...
                if (files.imageUrl && files.imageUrl[0].filepath) {
                    ...
                    const publicId = `cause-image/${Date.now()}`;
                ...
                await causeModel.updateCause(id, fieldsToUpdate);
                res.status(200).json({
                    success: true,
    ...
    deleteCauseById: async (req, res) => {
        ...
        try {
            ...
            const deleted = await causeModel.deleteCause(id);
